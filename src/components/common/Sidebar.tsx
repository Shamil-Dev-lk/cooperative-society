...
const navItems: NavItem[] = [
  ...
  { to: '/notifications', label: 'Notifications', labelSi: 'දැනුම්දීම්', icon: <Bell size={20} />, badge: true },
...
const SidebarContent: React.FC<SidebarContentProps> = ({
  ...
  const unreadNum = useNotificationStore((s) => s.unreadCount());
      ...
      <nav className="flex-1 py-4 overflow-y-auto overflow-x-hidden">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className="flex-shrink-0 flex items-center justify-center w-6 relative">
                {item.icon}
                {!sidebarOpen && item.badge && unreadNum > 0 && (
              ...
              <AnimatePresence>
                {sidebarOpen && (
                  ...
                  >
                    ...
                    {item.badge && unreadNum > 0 && (
                      <span className="bg-yellow-400 text-gray-950 font-bold text-[10px] px-2 py-0.5 rounded-full ml-2 shadow-sm animate-pulse">
                        {unreadNum}
                      </span>
                    )}
                  </motion.div>
