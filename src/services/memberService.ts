...
function parseDateSearch(search: string): string[] {
  ...
  // Normalize separators (spaces, dots, slashes) to hyphens
  const normalized = clean.replace(/[\s./]+/g, '-');

  // Pattern 1: YYYY-MM-DD
  if (/^\d{4}-\d{2}-\d{2}$/.test(normalized)) {
    return [joined_date.eq.${normalized}];
  }

  // Pattern 2: DD-MM-YYYY (reformat to YYYY-MM-DD)
  ...
  if (/^\d{4}-\d{2}$/.test(normalized)) {
    ...
    return [and(joined_date.gte.${year}-${month}-01,joined_date.lte.${year}-${month}-${lastDay.toString().padStart(2, '0')})];
  }

  // Pattern 4: MM-YYYY (reformat to YYYY-MM)
...
export const memberService = {
  async getMembers(
    ...
    if (filters.search) {
      ...
      const searchTerms = [
        member_no.ilike."%${cleanSearch}%",
        name.ilike."%${cleanSearch}%",
        nic.ilike."%${cleanSearch}%",
