...
const oriPath = 'C:\\Users\\USER\\Downloads\\01 නික-උතුර  Ori.xlsx';

async function run() {
  const divisionId = '24203ac6-fff0-46f2-9a71-a39b55ca6b06'; // Exact ID for නික - උතුර
  console.log(`=== Step 1: Deleting existing members in Division: නික - උතුර (${divisionId}) ===`);
  ...
  if (deleteErr) {
    console.log('Error deleting members:', deleteErr.message);
    return;
  }
  console.log(`Deleted ${deletedCount || 0} existing members from Division නික - උතුර.`);

  console.log(`=== Step 2: Reading Excel File: 01 නික-උතුර  Ori.xlsx ===`);

  if (!fs.existsSync(oriPath)) {
    console.log('Excel file not found:', oriPath);
    return;
  }
  ...
  const wb = XLSX.readFile(oriPath);
  ...
  console.log(`Extracted ${membersToUpload.length} members from 01 නික-උතුර  Ori.xlsx.`);
  ...
  console.log(`🎉 HIGH-SPEED ORI EXCEL UPLOAD COMPLETE! ${uploaded} Members Uploaded to Division: නික - උතුර`);
}
